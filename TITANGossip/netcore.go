...
func HandleIncomingConn(nConn net.Conn, config *ssh.ServerConfig, IsUserAllowedKeyAuth map[string]bool) {
	...
	for newChannel := range chans {
		if newChannel.ChannelType() != "keys" && newChannel.ChannelType() != "titangossip" && newChannel.ChannelType() != "nodeid" {
		...
		} else if newChannel.ChannelType() == "titangossip" {
...
func ConnectToPeer(P *Peer) error {
	...
	Chan, requests, err := client.OpenChannel("titangossip", nil)
	if err != nil {
		client.Close()
		return err
	}
